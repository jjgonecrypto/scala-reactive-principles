...
class Step5_PrimaryPersistenceSpec extends TestKit(ActorSystem("Step5PrimaryPersistenceSpec"))
  ...
  test("case5: Primary acknowledges only after persistence and global acknowledgement") {
    val arbiter = TestProbe()
    val persistence = TestProbe()
        val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = false)), "case6-primary")
  ...
  test("case6: Primary acknowledges only after persistence and global acknowledgement and flaky") {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case5-primary")
    val secondaryA, secondaryB = TestProbe()
    val client = session(primary)
