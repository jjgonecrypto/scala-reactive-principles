...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  def receiveN(requester: TestProbe, ops: Seq[Operation], expectedReplies: Seq[OperationReply]): Unit =
    requester.within(5.seconds) {
      val repliesUnsorted = for (i <- 1 to ops.size) yield try {
        ...
        case ex: Throwable if ops.size > 10 => fail(s"failure to receive confirmation ${ops(i)} ${i}/${ops.size}", ex)
  ...
  test("longer inserts and lookups") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
  ...
  test("no insert for 0 then insert then remove") {
    val topNode = system.actorOf(Props[BinaryTreeSet])

    topNode ! Contains(testActor, id = 10, 0)

    expectMsg(ContainsResult(10, false))

    topNode ! Insert(testActor, id = 20, 0)

    expectMsg(OperationFinished(20))

    topNode ! Contains(testActor, id = 30, 0)

    expectMsg(ContainsResult(30, true))

    topNode ! Remove(testActor, id = 40, 0)

    expectMsg(OperationFinished(40))

    topNode ! Contains(testActor, id = 50, 0)
  ...
  test("more thorough insert, contains and remove") {
    val requester = TestProbe()
    val requesterRef = requester.ref
    val ops = List(
      Insert(requesterRef, 1, -5),
      Insert(requesterRef, 2, 5),
      Insert(requesterRef, 3, 10),
      Insert(requesterRef, 4, 7),
      Insert(requesterRef, 5, 6),
      Contains(requesterRef, 6, 5),
      Remove(requesterRef, 7, 5),
      Contains(requesterRef, 8, 5)
    )
