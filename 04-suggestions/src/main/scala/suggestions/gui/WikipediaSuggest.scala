...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTermField.textValues.concatRecovered(s=> wikiSuggestResponseStream(s))
    ...
    val selections: Observable[String] = button.clicks.flatMap( _ => Observable.from(suggestionList.selection.items))

    val pages: Observable[Try[String]] = selections.concatRecovered(s => wikiPageResponseStream(s))
